...
$(function () {
    $.getJSON("../../config/config.json", function (json) {
        ...
        $('.add-area .add-btn-area .add-btn').on('click', function () {
            ...
            $.ajax({
                url: "http://" + json.urls + "/users/signup",
        ...
        $('.userT-add, .userT-delete').on('click', function () {
            addReset();
        });
        ...
        $('.userT-change').on('click', function () {
            ...
            let selId = $("#" + selRowIds).children('td[aria-describedby="userGrid_id"]').text();
            ...
            if (selRowIds.length == 1) {
                ...
                $("input[name='userid2']").attr("placeholder", $("input[name='userid2']").val()).val(selId).focus().blur();
            ...
            $('.add-area .add-btn-area .change-btn').on('click', function () {
                ...
                $.ajax({
                    url: "http://" + json.urls + "/users/" + selRowIds + "/update",
                    headers: { Authorization: "Bearer " + sessionStorage.getItem("Bearer") },
                    contentType: "application/json; charset=UTF-8",
                    method: "PUT",
                    dataType: "JSON",
